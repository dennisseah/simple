...
var template = '<p class="cmd-title">@@main-cmd@@</p><p class="cmd-description">@@cmd-description@@</p><p>&nbsp;</p><p>Options:</p>@@options@@<p>&nbsp;</p>';
var optionTemplate = '<p>@@option@@</p><p class="cmd-description">@@description@@</p><div class="line-space"></div>';
...
function showDetails(key) {
  ...
  var content = template.replace("@@main-cmd@@", "spk " + key + alias + sanitize(values) + " [options]");
  content = content.replace("@@cmd-description@@", cmd.description);

  var options = cmd.options.reduce(function(a, c) {
    a += optionTemplate.replace(
      "@@option@@", sanitize(c.arg)
    ).replace(
      "@@description@@", sanitize(c.description)
    );
    return a;
  }, "");
  options += optionTemplate.replace(
    "@@option@@", "-h, --help"
  ).replace(
  ...
  if (cmd.markdown) {
    content = "<div class=\"markdown\">" + converter.makeHtml(cmd.markdown) + "</div><hr>" + content;
  }
...
function populateListing() {
  ...
  var listing = cmdKeys.reduce(function(a, c) {
    a += "<li><a href=\"javascript:showDetails('" + c + "');\">spk " + c + "</a></li>";
    return a;
  }, "");
  ...
  if (listing) {
    ...
    $('#command-list').html("<span class=\"small-font\">no matching commands</span>");
  }
